  ...
  return (
    ...
    >
      ...
      {state.isOpen && (
        <div
          class={[
            "animate-in slide-in-from-bottom-5 fade-in fixed right-4 z-[100] flex h-[32rem] max-h-[80vh] w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#001407]/95 backdrop-blur-xl shadow-2xl text-white duration-300 sm:right-6 sm:w-85",
        ...
        >
          {/* Header */}
          <div class="flex items-center justify-between bg-slate-950/80 border-b border-white/5 p-5 text-white">
            <div class="flex items-center gap-3">
              <div class="relative">
                {props.avatarUrl ? (
                  ...
                  <div class="flex h-10 w-10 items-center justify-center rounded-full bg-slate-900 border border-white/10 p-1">
                    <img
                      src={logoMobile}
                      alt="Logo Sport Garden"
          ...
          <div
            ...
            class="flex flex-1 flex-col space-y-4 overflow-y-auto bg-[#001407]/30 p-4"
          >
            {state.messages.map((msg, i) => (
              ...
              >
                <div
                  class={[
                    "max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-sm font-medium",
                    msg.role === "user"
            ...
            {state.isLoading && (
              <div class="flex justify-start">
                <div class="flex items-center gap-1.5 rounded-2xl rounded-bl-none border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-400 shadow-sm">
          ...
          {/* Input Area */}
          <div class="border-t border-white/5 bg-slate-950/40 p-4">
            <form
            ...
            >
              <input
                ...
                class="flex-1 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder-slate-400 transition-all focus:border-emerald-500 focus:bg-white/10 focus:outline-none"
                disabled={state.isLoading}
              />
              <button
                type="submit"
                disabled={!inputValue.value.trim() || state.isLoading}
                class="flex items-center justify-center rounded-xl bg-emerald-500 p-3 text-white transition-all active:scale-95 disabled:bg-white/5 disabled:text-white/20 disabled:opacity-50"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
