...
export const HomeBookingModal = component$<HomeBookingModalProps>(
  ({
    ...
    // When true, the success screen is dismissed so the user can start a new booking
    const dismissedSuccess = useSignal(false);
    ...
    // Reset the wizard so the user can immediately request another booking
    // from the success screen, keeping the modal open.
    ...
    // Whether the post-submit success screen should replace the wizard form.
    const showSuccessScreen = useComputed$(() => {
      if (dismissedSuccess.value) return false;
      const g = guestAction.value;
      const u = userAction.value;
      const guestDone =
        g?.success &&
        (g.paymentMethod !== "PAYWAY" ||
          g.amountToCharge === 0 ||
          paywayPaymentSuccess.value);
      const userDone =
        u?.success &&
        (u.paymentMethod !== "PAYWAY" ||
          u.amountToCharge === 0 ||
          paywayPaymentSuccess.value);
      return !!(guestDone || userDone);
    });
                ...
                <div class="mx-auto max-w-md py-8">
                  {guestAction.value?.success && !paywayPaymentSuccess.value ? (
                    isTransfer(guestAction.value.paymentMethod) ? (
                      ...
                      <div class="space-y-4">
                        <div class="text-center">
                          <div class="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl border border-amber-500/20 bg-amber-500/10 text-amber-400 shadow-lg">
                            <svg
                              viewBox="0 0 24 24"
                              class="h-7 w-7 fill-[#25D366]"
                              xmlns="http://www.w3.org/2000/svg"
                            >
                              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.884-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                            </svg>
                        ...
                        <div class="overflow-hidden rounded-2xl border border-white/8 bg-slate-900/60">
                          <div class="border-b border-white/5 px-4 pt-3 pb-2.5">
                            <p class="text-[9px] font-black tracking-widest text-slate-500 uppercase">Resumen de tu reserva</p>
                          </div>
                          <div class="space-y-2 p-4">
                            <div class="flex justify-between text-xs">
                              <span class="text-slate-400">Cancha</span>
                              <span class="font-bold text-white">{selectedPitch.value?.name || "—"}</span>
                            </div>
                            <div class="flex justify-between text-xs">
                              <span class="text-slate-400">Fecha</span>
                              <span class="font-bold text-white">{dateStr.value ? dateStr.value.split("-").reverse().join("/") : "—"}</span>
                            </div>
                            <div class="flex justify-between text-xs">
                              <span class="text-slate-400">Horario</span>
                              <span class="font-bold text-emerald-400">{timeStr.value} hs · {durationStr.value} min</span>
                            </div>
                            {selectedExtras.value.map((e) => (
                              <div key={e.name} class="flex justify-between text-xs">
                                <span class="text-slate-400">{e.name}</span>
                                <span class="font-bold text-slate-300">+${Number(e.price).toLocaleString("es-AR")}</span>
                              </div>
                            ))}
                            <div class="flex justify-between border-t border-white/5 pt-2 text-xs">
                              <span class="font-black text-slate-300">Total</span>
                              <span class="font-black text-white">${totalPrice.value.toLocaleString("es-AR")}</span>
                            </div>
                          </div>
                          <div class="border-t border-white/5 px-4 py-2.5">
                            <p class="text-[10px] font-medium text-slate-500">
                              ...
                              {" "}— si está mal, hacé una nueva solicitud.
                            </p>
                          </div>
                        </div>

                        {/* Próximos pasos */}
                        <div class="space-y-2 rounded-xl border border-white/5 bg-slate-900/40 px-4 py-3">
                          <div class="flex items-center gap-2.5">
                            <div class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-amber-500/15 text-[9px] font-black text-amber-400">1</div>
                            <p class="text-[11px] font-medium text-slate-400">Tu reserva está en estado <span class="font-bold text-amber-400">Pendiente</span>.</p>
                          </div>
                          <div class="flex items-center gap-2.5">
                            <div class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#25D366]/10">
                              <svg class="h-2.5 w-2.5 fill-[#25D366]" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.884-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" /></svg>
                            </div>
                            <p class="text-[11px] font-medium text-slate-400">Te contactamos por <span class="font-bold text-[#4ADE80]">WhatsApp</span> para coordinar el pago.</p>
                          </div>
                          <div class="flex items-center gap-2.5">
                            <div class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 text-[9px] font-black text-emerald-400">✓</div>
                            <p class="text-[11px] font-medium text-slate-400">¡Tu turno queda <span class="font-bold text-white">Confirmado</span>!</p>
                          </div>
                        </div>
                        ...
                        <div class="space-y-2.5">
                          <button
                            ...
                            class="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 py-3.5 text-sm font-black tracking-wider text-white uppercase shadow-lg shadow-emerald-500/20 transition-all hover:bg-emerald-600 active:scale-[0.98]"
                          >
                          ...
                          {settings?.whatsappNumber && (
                            <a
                              ...
                              target="_blank"
                              class="flex w-full items-center justify-center gap-2 rounded-xl border border-[#25D366]/25 bg-[#25D366]/10 py-3 text-xs font-black tracking-wider text-[#4ADE80] uppercase transition-all hover:bg-[#25D366]/20 active:scale-[0.98]"
                            >
                              <svg class="h-4 w-4 fill-[#25D366]" viewBox="0 0 24 24">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.884-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                              </svg>
                  ...
                  {/* "Nueva reserva" shown for all success variants except guest (which has it inline) */}
                  {!guestAction.value?.success && (
                    <button
                      type="button"
                      onClick$={resetForNewBooking}
                      class="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 py-3.5 text-sm font-black tracking-wider text-white uppercase shadow-lg shadow-emerald-500/20 transition-all hover:bg-emerald-600 active:scale-[0.98]"
                    >
                          ...
                          <>
                            ...
                            <div>
                                  ...
                                  (extra) => {
                                    ...
                                    return (
                                      <button
                                        ...
                                        class={[
                                          "relative flex flex-col items-center gap-1.5 rounded-xl border-2 p-3 transition-all active:scale-95",
                                          isSelected
                                            ? "scale-[1.03] border-emerald-400 bg-emerald-500/15 text-emerald-300 shadow-[0_0_0_3px_rgba(16,185,129,0.18)]"
                                            : "bg-slate-850 border-white/5 text-slate-400 hover:border-emerald-500/40 hover:bg-slate-800",
                                        ]}
                                      >
                                        {isSelected && (
                                          <span class="absolute -top-2 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-emerald-500 text-white shadow-md shadow-emerald-900/50">
                                            <svg
                                              class="h-3 w-3"
                                              fill="none"
                                              viewBox="0 0 24 24"
                              ...
                              {selectedExtras.value.length > 0 && (
                                <div class="mt-3 flex flex-wrap items-center gap-2 rounded-xl border border-emerald-500/25 bg-emerald-500/8 px-3.5 py-2.5">
                                  <span class="shrink-0 text-[9px] font-black tracking-widest text-emerald-500 uppercase">Seleccionados:</span>
                                  {selectedExtras.value.map((e) => (
                                    <span key={e.name} class="flex items-center gap-1 rounded-full border border-emerald-500/30 bg-emerald-500/15 px-2.5 py-1 text-[10px] font-black text-emerald-300">
                                      <svg class="h-2.5 w-2.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3.5" d="M5 13l4 4L19 7" /></svg>
                                      {e.name} +${Number(e.price).toLocaleString("es-AR")}
                                    </span>
                                  ))}
                                  <span class="ml-auto text-xs font-black text-white">+${extrasTotal.value.toLocaleString("es-AR")}</span>
                                </div>
                              )}
                            </div>
                            ...
                            <div class="bg-slate-850/60 mt-4 space-y-5 rounded-2xl border border-white/5 p-5">
                                  ...
                                  {[
                                    ...
                                    return (
                                      <label
                                        key={pm.id}
                                        class={[
                                          "relative flex flex-col items-center justify-center cursor-pointer rounded-2xl border-2 p-3.5 text-center transition-all select-none active:scale-[0.98]",
                                          isSelected
                                            ? "border-emerald-400 bg-emerald-500/15 text-white shadow-[0_0_0_3px_rgba(16,185,129,0.18)]"
                              ...
                              {paymentMethod.value && (
                                <div class="flex items-center gap-2.5 rounded-xl border border-white/5 bg-slate-800/60 px-3.5 py-2.5">
                                  <div class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-500 text-white">
                                  ...
                                  <p class="text-[10px] font-bold text-slate-300">
                                    {isTransfer(paymentMethod.value)
                                      ? "Realizás la transferencia y enviás el comprobante por WhatsApp."
                                      : paymentMethod.value === "PAYWAY"
                                        ? "Pagarás con tarjeta de forma segura al confirmar la reserva."
                                        : paymentMethod.value === "MERCADOPAGO" || paymentMethod.value === "MERCADO_PAGO"
                                          ? "Se abrirá el checkout de Mercado Pago al confirmar."
                                          : paymentMethod.value === "CURRENT_ACCOUNT" || paymentMethod.value === "CUENTA_CORRIENTE"
                                            ? "Se cargará a tu cuenta corriente en el club."
                                            : "Abonás en efectivo el día del partido en el club."}
                                  </p>
                                </div>
                              )}
                                    ...
                                    {senaAmount.value > 0 && (
                                      <label
                                        class={[
                                          ...
                                          paymentOption.value === "SENA"
                                            ? "border-emerald-400 bg-emerald-500/15 shadow-[0_0_0_3px_rgba(16,185,129,0.12)]"
